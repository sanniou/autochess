...
func _ready():
	...
	# 连接信号
	EventBus.chess.connect_event("chess_piece_ability_activated", _on_ability_activated)
...
func _activate_all_abilities() -> void:
	for piece in created_pieces:
		if is_instance_valid(piece):
			...
			if piece.has_method("has_ability"):
				has_ability = piece.has_ability()
			elif piece.data and piece.data.has("ability"):
				has_ability = true

			if has_ability:
				# 设置满法力值
				if piece.has_method("set_property"):
					var max_mana = piece.get_property("max_mana")
					piece.set_property("current_mana", max_mana)
				elif piece.data:
					piece.data.current_mana = piece.data.max_mana
				...
				# 激活技能
				if piece.has_method("cast_ability"):
					piece.cast_ability()
				elif piece.has_method("activate_ability"):
	...
	for other_piece in created_pieces:
		if other_piece != piece and is_instance_valid(other_piece):
			...
			if piece.has_method("get_board_position"):
				piece_pos = piece.get_board_position()
			elif piece.has("board_position"):
				piece_pos = piece.board_position

			if other_piece.has_method("get_board_position"):
				other_pos = other_piece.get_board_position()
			elif other_piece.has("board_position"):
				other_pos = other_piece.board_position

			var distance = piece_pos.distance_to(other_pos)
...
func _on_cell_clicked(position: Vector2i) -> void:
	...
	if existing_piece:
		# 如果已有棋子，激活其技能
		# 设置满法力值
		if existing_piece.has_method("set_property"):
			var max_mana = existing_piece.get_property("max_mana")
			existing_piece.set_property("current_mana", max_mana)
		elif existing_piece.data:
			existing_piece.data.current_mana = existing_piece.data.max_mana

		# 查找目标
		var target = _find_target_for_piece(existing_piece)
		if target:
			existing_piece.set_target(target)

		# 激活技能
		if existing_piece.has_method("cast_ability"):
			existing_piece.cast_ability()
		elif existing_piece.has_method("activate_ability"):
...
func _on_ability_activated(piece: ChessPiece, target) -> void:
	...
	print("棋子 " + display_name + " 激活了技能: " + ability_name)
