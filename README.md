...
1. **游戏测试和调试系统实现**
   - 实现了单元测试框架，用于测试游戏的各个组件
   - 实现了错误报告系统，用于收集和报告游戏中的错误
   - 实现了性能分析工具，用于分析游戏性能并生成报告

2. **游戏本地化系统完善**
   - 扩展了本地化管理器，支持更多语言和更高级的本地化功能
   - 实现了自动检测系统语言和切换语言的功能
   - 添加了字体支持，根据不同语言自动加载相应字体

3. **游戏发布准备工作**
   - 实现了游戏测试和调试系统，提高游戏质量
   - 完善了游戏本地化系统，支持多语言
   - 准备实现游戏发布系统，包括版本更新、游戏打包和发布渠道集成
...
1. **游戏逻辑系统**
   - 棋子系统：
     - 棋子基础类已完成
     - 技能系统已完成，包括多种技能类型（伤害、治疗、增益、区域伤害、连锁、光环、召唤、传送）
     - 羁绊系统已完成，支持元素和召唤羁绊效果
     - 元素效果系统已完成，包括火、冰、雷、土元素效果
   - 棋盘系统：
     - 基础棋盘框架已完成
     - 拖放交互已完成，支持动画和音效
     - 格子高亮系统已完成，支持多种高亮效果
   ...
   - 战斗系统：
     - 基础战斗框架已完成
     - 战斗AI已实现，支持多种难度和行为类型
     - 战斗结算机制已完成，支持根据表现调整奖励
     - 战斗速度控制已实现
     - 战斗统计和结果显示已实现
   - 装备系统：
     - 基础装备类已完成
     - 装备效果系统已完成，支持多种效果类型（被动、触发、闪避、暴击、元素、生命值百分比）
     - 装备合成系统已完成
   - 遗物系统：已完成，包括遗物管理器、遗物基类、遗物效果实现和遗物UI界面
   - 地图系统：
     - 基础地图框架已完成
     - 地图生成算法已实现，支持多种模板和难度
     - 节点交互已实现，支持悬停提示和视觉反馈
     - 路径选择已实现，支持路径预览和最佳路径显示
     - 连接线动画已实现，提供流动效果
...
1. **状态效果系统完善**
   - 完善状态效果管理器中缺失的视觉效果实现（减速、燃烧、中毒、流血、增益、减益等）
   - 优化状态效果的应用和移除逻辑，确保效果正确叠加和移除
   - 添加更多视觉反馈，提升游戏体验

2. **技能系统扩展**
   - 扩展现有技能类型，增加更多变化和组合可能
   - 优化技能目标选择逻辑，提高AI决策合理性
   - 完善技能视觉效果和动画，增强战斗表现力

3. **地图系统完善**
   - 增加更多节点类型和事件内容
   - 优化地图生成算法，提供更多样化的路径选择
   - 完善节点交互和视觉效果
...
3. 优先使用已有文件，如果判断已有文件不合理可以直接修改/删除/替换。不要随意新建相同功能的新文件/函数，以免造成混乱。
4. 不合理的或者有更好实现的模块请直接用最优解重构，不需要考虑兼容问题，重构完成后检查api的变化，修复其他引用此模块的地方。
5. 我们生成代码的时候，提前考虑好性能、可扩展性、解耦、模块化的实现。代码要提前关注可能与其他系统的交互,预设交互空间（比如难度、皮肤、多语言-目前只需要中文、成就统计、遗物影响、事件影响、插件系统以及其它系统）
6. 当涉及到图片、音频、模型等你无法生成的文件的时候，请使用占位（或者godot 是否会有一些公共素材？）。
7. 新增/删除/重构功能时判断此项目是否真的需要这些需修改？在合适的范围内做减法。
8. 完成模块开发后，在最后再次确认是否有遗漏，确认无误后更新 README.md，以规划下一步的开发工作。
