...
func _ready():
	# 示例 1: 获取所有棋子配置
	print("示例 1: 获取所有棋子配置")
	var all_chess_pieces = config_manager.get_all_chess_pieces()
	print("棋子数量: ", all_chess_pieces.size())
	
	# 示例 2: 获取特定棋子配置
	print("\n示例 2: 获取特定棋子配置")
	var warrior = config_manager.get_chess_piece_config("warrior_1")
	...
	# 示例 3: 获取所有遗物配置
	print("\n示例 3: 获取所有遗物配置")
	var all_relics = config_manager.get_all_relics()
	print("遗物数量: ", all_relics.size())
	
	# 示例 4: 获取特定遗物配置
	print("\n示例 4: 获取特定遗物配置")
	var lucky_coin = config_manager.get_relic_config("lucky_coin")
	...
	# 示例 5: 获取所有事件配置
	print("\n示例 5: 获取所有事件配置")
	var all_events = config_manager.get_all_events()
	print("事件数量: ", all_events.size())
	
	# 示例 6: 获取特定事件配置
	print("\n示例 6: 获取特定事件配置")
	var treasure_chest = config_manager.get_event_config("treasure_chest")
	...
	# 示例 7: 使用通用方法获取配置
	print("\n示例 7: 使用通用方法获取配置")
	var chess_config = config_manager.get_config("chess_pieces")
	print("棋子数量: ", chess_config.size())
	
	# 示例 8: 加载任意配置文件
	print("\n示例 8: 加载任意配置文件")
	var custom_config = config_manager.load_json("res://config/chess_pieces.json")
