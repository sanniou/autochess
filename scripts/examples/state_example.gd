...
func _ready():
	...
	state_manager.subscribe("game", self, "_on_game_state_changed")
	state_manager.subscribe("ui", self, "_on_ui_state_changed")
	
	# 获取初始状态
	player_state = state_manager.get_state_section("player")
	game_state = state_manager.get_state_section("game")
	ui_state = state_manager.get_state_section("ui")
...
func _toggle_pause():
	# 创建动作
	var action = state_manager.create_action("SET_PAUSED", {"is_paused": not game_state.is_paused})
...
func _show_notification(message: String, type: String = "info"):
	# 创建动作
	var action = state_manager.create_action("ADD_NOTIFICATION", {"message": message, "type": type})
...
func _on_player_state_changed(new_state):
	...
	if player_state.health <= 0 and not game_state.is_game_over:
		# 游戏结束
		var action = state_manager.create_action("SET_GAME_OVER", {"is_game_over": true, "win": false})
		state_manager.dispatch(action)
