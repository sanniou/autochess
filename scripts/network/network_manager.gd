...
func create_server() -> bool:
	...
	EventBus.debug_message.emit("服务器已创建，端口: " + str(network_config.server_port), 0)
	return true
...
func connect_to_server(address: String, port: int = -1) -> bool:
	...
	EventBus.debug_message.emit("正在连接到服务器: " + address + ":" + str(port), 0)
	return true
...
func close_connection() -> void:
	...
	EventBus.debug_message.emit("网络连接已关闭", 0)
...
func set_network_config(config: Dictionary) -> void:
	...
	EventBus.debug_message.emit("网络配置已更新", 0)
...
func _reconnect() -> void:
	...
	EventBus.debug_message.emit("尝试重连 (" + str(_reconnect_attempts) + "/" + str(network_config.reconnect_attempts) + ")", 0)
...
func _setup_upnp() -> void:
	...
	if discover_result != UPNP.UPNP_RESULT_SUCCESS:
		EventBus.debug_message.emit("UPNP发现失败: " + str(discover_result), 1)
		return
	
	# 获取网关
	var gateway = upnp.get_gateway()
	if gateway == null:
		EventBus.debug_message.emit("无法获取UPNP网关", 1)
		return
	
	# 映射端口
	var map_result = gateway.add_port_mapping(network_config.server_port, network_config.server_port, "GodotServer", "UDP")
	if map_result != UPNP.UPNP_RESULT_SUCCESS:
		EventBus.debug_message.emit("UPNP端口映射失败: " + str(map_result), 1)
		return
	
	EventBus.debug_message.emit("UPNP端口映射成功: " + str(network_config.server_port), 0)
...
func _close_upnp() -> void:
	...
	EventBus.debug_message.emit("UPNP端口映射已删除", 0)
...
func _on_peer_connected(id: int) -> void:
	...
	EventBus.debug_message.emit("玩家已连接: " + str(id), 0)
...
func _on_peer_disconnected(id: int) -> void:
	...
	EventBus.debug_message.emit("玩家已断开: " + str(id), 0)
...
func _on_connected_to_server() -> void:
	...
	EventBus.debug_message.emit("已连接到服务器，玩家ID: " + str(local_player_id), 0)
...
func _on_connection_failed() -> void:
	...
	EventBus.debug_message.emit("连接到服务器失败", 1)
...
func _on_server_disconnected() -> void:
	...
	EventBus.debug_message.emit("服务器已断开连接", 1)
