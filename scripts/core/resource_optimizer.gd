...
func clear_all_resources() -> void:
	...
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("所有资源已清除", 0))
...
func _unload_low_priority_resources() -> void:
	...
	if unload_count > 0:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("已卸载 " + str(unload_count) + " 个低优先级资源", 0))
...
func _unload_unused_resources() -> void:
	...
	if unload_count > 0:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("已卸载 " + str(unload_count) + " 个长时间未使用的资源", 0))
...
func enable_auto_unload() -> void:
	...
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("资源自动卸载已启用", 0))

## 禁用自动卸载
func disable_auto_unload() -> void:
	optimization_settings.auto_unload_enabled = false
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("资源自动卸载已禁用", 0))
...
func set_optimization_settings(settings: Dictionary) -> void:
	...
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("资源优化设置已更新", 0))
