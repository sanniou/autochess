...
static func dispatch_delayed(event: BusEvent, delay_seconds: float) -> void:
	var timer = Timer.new()
...
static func dispatch_when(event: BusEvent, condition: Callable, check_interval: float = 0.1) -> void:
	var timer = Timer.new()
	GlobalEventBus.event_bus.add_child(timer)
