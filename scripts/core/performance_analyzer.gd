...
func start_analysis() -> void:
	...
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("开始性能分析", 0))
...
func stop_analysis() -> Dictionary:
	...
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("停止性能分析", 0))
...
func enable_analysis() -> void:
	...
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("性能分析已启用", 0))

## 禁用性能分析
func disable_analysis() -> void:
	analysis_settings.enabled = false
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("性能分析已禁用", 0))

## 启用自动分析
func enable_auto_analyze() -> void:
	analysis_settings.auto_analyze = true
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("自动性能分析已启用", 0))

## 禁用自动分析
func disable_auto_analyze() -> void:
	analysis_settings.auto_analyze = false
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("自动性能分析已禁用", 0))
