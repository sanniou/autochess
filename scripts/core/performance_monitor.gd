...
func _log_performance_data() -> void:
	...
	# 输出日志
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new(log_message, 0))
...
func enable() -> void:
	...
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("性能监控已启用", 0))

# 禁用性能监控
func disable() -> void:
	monitor_settings.enabled = false
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("性能监控已禁用", 0))

# 设置日志间隔
func set_log_interval(interval: float) -> void:
	monitor_settings.log_interval = max(0.1, interval)
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("性能监控日志间隔已设置为 %.1f 秒" % monitor_settings.log_interval, 0))

# 设置样本数量
func set_sample_count(count: int) -> void:
	monitor_settings.sample_count = max(10, count)
	_initialize_performance_data()
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("性能监控样本数量已设置为 %d" % monitor_settings.sample_count, 0))

# 启用脚本性能分析
func enable_script_profiling() -> void:
	monitor_settings.script_profiling = true
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("脚本性能分析已启用", 0))

# 禁用脚本性能分析
func disable_script_profiling() -> void:
	monitor_settings.script_profiling = false
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("脚本性能分析已禁用", 0))
