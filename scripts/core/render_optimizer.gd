...
func set_optimization_level(level: int) -> void:
	...
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("渲染优化级别已设置为: " + _get_level_name(level), 0))
...
func set_culling_settings(settings: Dictionary) -> void:
	...
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("视口剔除设置已更新", 0))
...
func auto_detect_optimization_level() -> void:
	...
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("自动检测优化级别: " + _get_level_name(level), 0))
