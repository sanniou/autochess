...
func _process(delta):
	...
	if autosave_enabled and current_save_slot != "" and GameManager.current_state != GameManager.GameState.NONE and GameManager.current_state != GameManager.GameState.MAIN_MENU:
		...
		if _autosave_timer <= 0:
			...
			GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("游戏已自动存档", 0))
...
func load_save_metadata() -> void:
	...
	if metadata.is_empty():
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法加载存档元数据", 2))
		return
	...
	if not save_metadata.has("version") or save_metadata.version != SAVE_VERSION:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("存档版本不兼容，可能需要迁移", 1))
...
func _save_metadata() -> void:
	...
	if not result:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法写入存档元数据文件", 2))
...
func save_game(slot_name: String = "") -> bool:
	...
	if slot_name == "":
		...
		if slot_name == "":
			GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("未指定存档槽", 2))
			return false
	...
	if not result:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法写入存档文件: " + save_path, 2))
		return false
...
func load_game(slot_name: String) -> bool:
	...
	if not FileAccess.file_exists(save_path):
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("存档文件不存在: " + save_path, 2))
		return false

	# 使用 ConfigManager 加载存档文件
	var save_data = GameManager.config_manager.load_json(save_path)
	if save_data.is_empty():
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法加载存档文件: " + save_path, 2))
		return false

	# 版本兼容性检查
	if not save_data.has("version") or save_data.version != SAVE_VERSION:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("存档版本不兼容，可能无法正确加载", 1))
...
func delete_save(slot_name: String) -> bool:
	...
	if dir.remove(slot_name + SAVE_EXTENSION) != OK:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法删除存档文件: " + save_path, 2))
		return false
...
func _apply_player_save_data(data: Dictionary) -> void:
	...
	if player_manager and not data.is_empty():
		...
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("应用玩家存档数据成功", 0))
	else:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法应用玩家存档数据", 1))
...
func _apply_map_save_data(data: Dictionary) -> void:
	...
	if map_manager and not data.is_empty():
		...
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("应用地图存档数据成功", 0))
	else:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法应用地图存档数据", 1))
...
func _apply_chess_pieces_save_data(data: Array) -> void:
	...
	if chess_manager and data.size() > 0:
		...
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("应用棋子存档数据成功", 0))
	else:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法应用棋子存档数据", 1))
...
func _apply_equipment_save_data(data: Array) -> void:
	...
	if equipment_manager and data.size() > 0:
		...
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("应用装备存档数据成功", 0))
	else:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法应用装备存档数据", 1))
...
func _apply_relics_save_data(data: Array) -> void:
	...
	if relic_manager and data.size() > 0:
		...
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("应用遗物存档数据成功", 0))
	else:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法应用遗物存档数据", 1))
...
func _apply_achievements_save_data(data: Dictionary) -> void:
	...
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("应用成就存档数据", 0))
...
func _apply_tutorials_save_data(data: Dictionary) -> void:
	...
	GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("应用教程存档数据", 0))
...
func _apply_settings_save_data(data: Dictionary) -> void:
	...
	if settings_manager and not data.is_empty():
		...
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("应用设置存档数据成功", 0))
	else:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法应用设置存档数据", 1))
...
func save_achievement_data(data: Dictionary) -> bool:
	...
	if not result:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法写入成就数据文件: " + save_path, 2))
		return false
...
func load_achievement_data() -> Dictionary:
	...
	if data.is_empty():
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法加载成就数据文件: " + save_path, 2))
		return {}
...
func save_tutorial_data(data: Dictionary) -> bool:
	...
	var result = GameManager.config_manager.save_json(save_path, data)
	if not result:
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法写入教程数据文件: " + save_path, 2))
		return false
...
func load_tutorial_data() -> Dictionary:
	...
	var data = GameManager.config_manager.load_json(save_path)
	if data.is_empty():
		GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("无法加载教程数据文件: " + save_path, 2))
		return {}
...
func _on_game_state_changed(old_state, new_state) -> void:
	...
	if old_state == GameManager.GameState.BATTLE and new_state != GameManager.GameState.GAME_OVER:
		if autosave_enabled and current_save_slot != "":
			save_game(AUTOSAVE_NAME)
			GlobalEventBus.debug.dispatch_event(DebugEvents.DebugMessageEvent.new("战斗结束后自动存档", 0))
