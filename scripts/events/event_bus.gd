...
func _log_error(message: String) -> void:
	...
	# 使用调试事件发送错误消息
	if debug:
		debug.debug_message.emit(message, 2)
...
func _log_warning(message: String) -> void:
	...
	# 使用调试事件发送警告消息
	if debug:
		debug.debug_message.emit(message, 1)
...
func _log_info(message: String) -> void:
	...
	# 使用调试事件发送信息消息
	if debug:
		debug.debug_message.emit(message, 0)
...
class EventGroup extends Node:
	...
	# 触发事件
	func emit_signal(signal_name: String, args: Array = []) -> void:
		# 调用父类的emit_signal方法
	...
	# 连接信号
	func connect(signal_name: String, callable: Callable, flags: int = 0) -> Error:
		# 如果信号不存在，添加信号
		if not has_signal(signal_name):
			add_signal(signal_name)
		
		# 调用父类的connect方法
		var result = super.connect(signal_name, callable, flags)
	...
	# 断开信号
	func disconnect(signal_name: String, callable: Callable) -> void:
		# 调用父类的disconnect方法
		super.disconnect(signal_name, callable)
	...
	# 获取信号连接列表
	func get_signal_connection_list(signal_name: String) -> Array:
		return super.get_signal_connection_list(signal_name)
	
	# 是否有信号
	func has_signal(signal_name: String) -> bool:
		return super.has_signal(signal_name)
	
	# 是否已连接
	func is_connected(signal_name: String, callable: Callable) -> bool:
		return super.is_connected(signal_name, callable)
