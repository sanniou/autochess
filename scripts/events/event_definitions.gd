...
class BattleEvents:
	...
	# 战斗行为
	const DAMAGE_DEALT = "damage_dealt"                    # (source, target, amount, damage_type)
	const HEAL_RECEIVED = "heal_received"                  # (target, amount, source)
	const ABILITY_USED = "ability_used"                    # (piece, ability_data)
	const ABILITY_EFFECT_APPLIED = "ability_effect_applied"  # (source, target, effect_type, effect_value)
	const UNIT_DIED = "unit_died"                          # (unit_data)
