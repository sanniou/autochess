...
RELICS_JSON_PATH = os.path.join(CONFIG_DIR, "relics/relics.json")
EQUIPMENT_JSON_PATH = os.path.join(CONFIG_DIR, "equipment.json")
CHESS_JSON_PATH = os.path.join(CONFIG_DIR, "chess.json")
...
def fix_relics_json():
    """修复 relics.json 文件"""
    print("开始修复 relics.json 文件...")
    
    # 加载 relics.json 文件
    relics_data = load_json_file(RELICS_JSON_PATH)
    if not relics_data:
        print("无法加载 relics.json 文件")
        return
    
    fixed_count = 0
    
    # 遍历所有遗物
    for relic_id, relic in relics_data.items():
        # 修复稀有度（确保是整数）
        if "rarity" in relic and isinstance(relic["rarity"], float):
            relic["rarity"] = int(relic["rarity"])
            fixed_count += 1
            print(f"修复遗物 {relic_id} 的稀有度: {relic['rarity']}")
        
        # 修复效果
        if "effects" in relic and isinstance(relic["effects"], list):
        ...
        # 修复触发条件
        if "trigger_conditions" in relic and isinstance(relic["trigger_conditions"], dict):
            new_trigger_conditions = {}
            for trigger, conditions in relic["trigger_conditions"].items():
                # 修复触发条件
                fixed_trigger = fix_trigger_type(trigger)
                if trigger != fixed_trigger:
                    fixed_count += 1
                
                # 添加到新的触发条件字典
                new_trigger_conditions[fixed_trigger] = conditions
            
            # 更新触发条件
            relic["trigger_conditions"] = new_trigger_conditions
    
    print(f"修复了 {fixed_count} 个问题")
    
    # 保存修复后的文件
    if save_json_file(RELICS_JSON_PATH, relics_data):
        print("修复完成，已保存 relics.json 文件")
    else:
        print("保存 relics.json 文件失败")
...
def fix_all_json_files():
    ...
    for json_file in json_files:
        if json_file == EQUIPMENT_JSON_PATH or json_file == RELICS_JSON_PATH:
            continue
        
        print(f"检查 {json_file} 文件...")
        ...
        def fix_json_object(obj):
            ...
            if isinstance(obj, dict):
                # 修复稀有度
                if "rarity" in obj and isinstance(obj["rarity"], float):
                    obj["rarity"] = int(obj["rarity"])
                    fixed_count += 1
                    modified = True
                
                # 修复效果
                if "effects" in obj and isinstance(obj["effects"], list):
                ...
                # 修复触发条件
                if "trigger_conditions" in obj and isinstance(obj["trigger_conditions"], dict):
                    new_trigger_conditions = {}
                    for trigger, conditions in obj["trigger_conditions"].items():
                        # 修复触发条件
                        fixed_trigger = fix_trigger_type(trigger)
                        if trigger != fixed_trigger:
                            fixed_count += 1
                    ...
                    # 更新触发条件
                    obj["trigger_conditions"] = new_trigger_conditions
                
                # 递归处理子对象
                for key, value in obj.items():
                    fix_json_object(value)
            
            elif isinstance(obj, list):
                # 递归处理列表中的每个元素
                for item in obj:
                    fix_json_object(item)
        
        # 修复 JSON 对象
        fix_json_object(data)
        
        # 如果有修改，保存文件
        if modified:
            print(f"修复了 {fixed_count} 个问题")
            if save_json_file(json_file, data):
                print(f"修复完成，已保存 {json_file} 文件")
            else:
                print(f"保存 {json_file} 文件失败")
        else:
            print(f"没有发现问题")
