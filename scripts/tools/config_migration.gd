...
func check_code_usage():
	...
	print("\n示例代码修改：")
	print("旧代码：")
	print("var config = GameManager.config_manager.get_chess_piece_config(\"warrior_1\")")
	print("新代码：")
	print("var config = GameManager.config_manager.get_chess_piece_config(\"warrior_1\")")
	print("注意：新的配置管理器保持了向后兼容的API，所以大多数代码不需要修改")
	
	print("\n旧代码：")
	print("var config_data = GameManager.config_manager.load_json(\"res://config/custom.json\")")
	print("新代码：")
	print("var config_data = GameManager.config_manager.load_json(\"res://config/custom.json\")")
	
	print("\n旧代码：")
	print("var all_chess = GameManager.config_manager.get_all_chess_pieces()")
	print("新代码：")
	print("var all_chess = GameManager.config_manager.get_all_chess_pieces()")
	...
	print("// 获取配置模型")
	print("var model = GameManager.config_manager.get_config_model(\"custom\", \"item_id\")")
	print("// 获取所有配置模型")
	print("var all_models = GameManager.config_manager.get_all_config_models(\"custom\")")
	print("// 设置配置项")
