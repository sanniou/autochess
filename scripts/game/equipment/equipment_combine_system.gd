...
func _create_combine_animation(equipment1: Equipment, equipment2: Equipment, result: Equipment) -> void:
	# 发送合成动画信号
	GlobalEventBus.equipment.dispatch_event(EquipmentEvents.EquipmentCombineAnimationStartedEvent.new(equipment1, equipment2, result))

	# 在合成动画完成后发送合成完成信号
	var timer = get_tree().create_timer(1.0) # 假设动画时间1秒
	timer.timeout.connect(func():
		GlobalEventBus.equipment.dispatch_event(EquipmentEvents.EquipmentCombineAnimationCompletedEvent.new(equipment1, equipment2, result))
	)
