...
# 	map_config = config_manager.get_map_config()
...
# 		available_node_types.append(type)
...
# 	return _weighted_choice(available_node_types, weights)
...
# 				"equipment": {
# 					"guaranteed": true,
# 					"quality": min(1 + floor(float(layer) / 2.0), 3)  # 装备品质随层数提升
# 				}
# 			}
...
# 				rewards["relic"] = {
# 					"guaranteed": true,
# 					"rarity": min(floor(float(layer) / 3.0), 2)  # 遗物稀有度随层数提升
# 				}
# 		"shop":
# 			# 商店节点没有直接奖励
# 			pass
...
# 				"equipment": {
# 					"guaranteed": true,
# 					"quality": min(2 + floor(float(layer) / 3.0), 4)  # 高品质装备
# 				},
# 				"relic": {
# 					"guaranteed": true,
# 					"rarity": min(1 + floor(float(layer) / 3.0), 3)  # 稀有遗物
# 				}
# 			}
# 		"mystery":
# 			# 神秘节点的奖励在触发时决定
