extends BaseShop
class_name MysteryShop
## 神秘商店
## 管理神秘商店的特定功能

# 神秘商店特有属性
var item_type_weights: Dictionary = {
	ShopItem.ItemType.CHESS: 0.1,
	ShopItem.ItemType.EQUIPMENT: 0.3,
	ShopItem.ItemType.RELIC: 0.5,
	ShopItem.ItemType.CONSUMABLE: 0.1
}

# 稀有度提升
var rarity_boost: float = 0.3
...
func _init(generators, calculators):
	super(null, null)
	shop_name = "MysteryShop"
	shop_type = ShopConstants.ShopType.MYSTERY_SHOP
	max_items = 4 # 神秘商店默认4个物品
...
func _generate_items(player_level: int) -> Array:
	...
	for item_type in type_counts:
		...
		if generator and calculator:
			# 生成物品
			var options = {
				"is_mystery_shop": true,
				"rarity_boost": rarity_boost, # 神秘商店稀有度提升
				"quality_boost": 0.3 # 神秘商店品质提升
			}
			...
			for item in type_items:
				...
				var price_multiplier = 1.2 # 神秘商店价格倍率
				item.set_cost(int(base_price * price_multiplier * discount_rate))
...
func _weighted_random_type() -> int:
	...
	return ShopItem.ItemType.RELIC # 默认返回遗物类型
...
func sync_economy_params() -> void:
	...
	# 难度越高，高级物品（遗物）出现概率越高
	item_type_weights[ShopItem.ItemType.CHESS] = 0.1 - difficulty * 0.01
	item_type_weights[ShopItem.ItemType.EQUIPMENT] = 0.3 - difficulty * 0.02
	item_type_weights[ShopItem.ItemType.RELIC] = 0.5 + difficulty * 0.03
	item_type_weights[ShopItem.ItemType.CONSUMABLE] = 0.1

	# 难度越高，稀有度提升越大
	rarity_boost = 0.3 + difficulty * 0.05
