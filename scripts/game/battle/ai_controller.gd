...
func _make_random_decision(piece: ChessPiece) -> void:
	...
	# 随机使用技能
	if piece.ability and piece.current_mana >= piece.ability_cost and randf() < 0.3:
		piece.use_ability()
...
func _make_aggressive_decision(piece: ChessPiece) -> void:
	...
	# 积极使用技能
	if piece.ability and piece.current_mana >= piece.ability_cost:
		piece.use_ability()
...
func _make_defensive_decision(piece: ChessPiece) -> void:
	...
	# 保守使用技能，优先保留法力值
	if piece.ability and piece.current_mana >= piece.ability_cost * 1.5 and randf() < 0.5:
		piece.use_ability()
...
func _make_balanced_decision(piece: ChessPiece) -> void:
	...
	# 平衡使用技能
	if piece.ability and piece.current_mana >= piece.ability_cost and randf() < 0.6:
		piece.use_ability()
...
func _evaluate_ability_usage(piece: ChessPiece) -> bool:
	# 如果没有技能或法力值不足，不使用
	if not piece.ability or piece.current_mana < piece.ability_cost:
		return false
