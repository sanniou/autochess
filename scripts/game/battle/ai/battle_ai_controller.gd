...
func update(delta: float, enemy_units: Array) -> void:
	...
	if decision_timer >= decision_interval:
		...
		for unit in enemy_units:
			if unit.current_state != unit.ChessState.DEAD and unit.current_state != unit.ChessState.STUNNED:
		...
		# 优化：使用简化的决策逻辑
		_update_simplified(delta, active_units)

# 简化的更新方法
func _update_simplified(delta: float, active_units: Array) -> void:
	# 根据难度决定每次处理的单位数量
	var units_per_update = 3  # 默认值
	...
	# 限制处理的单位数量
	var units_to_process = min(active_units.size(), units_per_update)

	# 随机选择单位进行处理
	active_units.shuffle()

	# 处理选定数量的单位
	for i in range(units_to_process):
		# 做出决策
		_make_simplified_decision(active_units[i])
...
# 简化的决策方法
func _make_simplified_decision(unit) -> void:
	# 检查单位是否被嘲讽
	if unit.taunted_by and is_instance_valid(unit.taunted_by):
		# 如果被嘲讽，直接选择嘲讽源作为目标
		unit.set_target(unit.taunted_by)
		...
		# 尝试使用技能
		_try_use_ability_simplified(unit)
		return
	...
	if difficulty <= AIDifficulty.NORMAL:
		...
		# 困难/专家难度：选择生命值最低的目标
		target = _find_lowest_health_target(player_units)
...
# 简化的技能使用方法
func _try_use_ability_simplified(unit) -> bool:
	# 如果没有技能或法力值不足，不使用
	...
	# 法力值越高，使用概率越大
	var mana_ratio = unit.current_mana / unit.max_mana
	use_chance *= (0.5 + mana_ratio * 0.5)  # 法力值满时提高50%概率
