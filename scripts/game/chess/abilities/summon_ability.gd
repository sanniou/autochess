...
func _execute_effect(target = null) -> void:
	...
	for i in range(min(summon_count, empty_cells.size())):
		...
		if summon_duration > 0:
			var timer = get_tree().create_timer(summon_duration)
			timer.timeout.connect(func(): _remove_summon(summon))
...
func _play_despawn_effect(summon: ChessPiece) -> void:
	...
	# 创建消失动画
	var tween = create_tween()
	tween.tween_property(effect, "scale", Vector2(0.1, 0.1), 0.5)
	tween.parallel().tween_property(effect, "modulate", Color(1, 1, 1, 0), 0.5)
	tween.tween_callback(effect.queue_free)
