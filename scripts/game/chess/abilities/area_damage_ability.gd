...
func _execute_effect(target = null) -> void:
	...
	for enemy in enemies:
		...
		if game_manager and game_manager.effect_manager:
			...
			# 使用特效管理器创建特效
			game_manager.effect_manager.create_effect(game_manager.effect_manager.EffectType.DAMAGE, enemy, params)
...
func _play_area_visual_effect(center_pos: Vector2i, radius: float) -> void:
	...
	# 使用特效管理器创建特效
	game_manager.effect_manager.create_effect(game_manager.effect_manager.EffectType.AREA_DAMAGE, effect_node, params)
