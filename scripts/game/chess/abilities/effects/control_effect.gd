...
func apply() -> void:
	...
	if randf() < resist_chance:
		...
		EventBus.status_effect.emit_event("status_effect_resisted", [target, effect_id])
		return
	...
	# 发送效果应用信号
	EventBus.battle.emit_event("ability_effect_applied", [source, target, "control", control_type])

	# 发送状态效果添加信号
	EventBus.status_effect.emit_event("status_effect_added", [target, effect_id, effect_data])
