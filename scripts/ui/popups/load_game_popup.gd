...
func _update_popup() -> void:
	...
	if has_node("Panel/TitleLabel"):
		get_node("Panel/TitleLabel").text = popup_data.get("title", tr("ui.load.title"))
...
func _on_delete_button_pressed() -> void:
	...
	# 显示确认对话框
	var message = tr("ui.save.delete_message").format({"save_name": save_name})
...
func _load_selected_save() -> void:
	...
	if success:
		# 显示成功提示
		EventBus.ui.emit_event("show_toast", [tr("ui.save.load_success"), 2.0])
		...
		# 显示失败提示
		EventBus.ui.emit_event("show_toast", [tr("ui.save.load_failed"), 2.0])
