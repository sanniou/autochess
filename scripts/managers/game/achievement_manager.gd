...
func _on_battle_ended(result) -> void:
	...
	if typeof(result) == TYPE_DICTIONARY:
		...
		# 如果是布尔值，创建简单的结果字典
		battle_result = BattleResult.create_simple(result).to_dict()
	elif result is BattleResult:
		# 如果是 BattleResult 对象，转换为字典
		battle_result = result.to_dict()
	else:
		# 无法处理的类型，返回
		_log_warning("_on_battle_ended 收到了无法处理的结果类型: " + str(typeof(result)))
		return
...
# 游戏完成事件处理
func _on_game_completed(victory: bool) -> void:
	if not victory:
		return
	...
	_log_info("游戏完成，游戏时间: " + str(game_time) + " 秒")
