...
# 注销UI节流器事件处理
func _on_unregister_ui_throttler(ui_id: String) -> void:
	if _throttlers.has(ui_id):
		_throttlers.erase(ui_id)

# 强制UI更新事件处理
func _on_force_ui_update(ui_id: String, component_id: String = "") -> void:
	force_update(ui_id, component_id)
