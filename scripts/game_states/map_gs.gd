...
func on_enter(_owner_node, _params: Dictionary = {}):
	...
	GlobalEventBus.gameflow.dispatch_event(GameFlowEvents.MapStateEnteredEvent.new())
	# GameManager._log_info("Dispatched MapStateEnteredEvent")

# Called when exiting the state
func on_exit(_owner_node):
	super.on_exit(_owner_node)
	# No specific MapStateExitedEvent in GameFlowEvents
	# GameManager._log_info("Exited MapGS")
	pass
